/**
 * AMPL Rebase Protection Monitor - Fixed Version
 * Actually monitors and protects against rebase issues
 */

class AMPLRebaseProtectionMonitor {
    constructor() {
    ...
    initialize() {
        console.log('🛡️ Initializing AMPL Rebase Protection Monitor...');
        ...
        if (this.targetPanel) {
            this.enhanceRebasePanel();
            this.applyStyles();
            this.bindEventListeners();
            this.loadRebaseData();
            this.startRebaseMonitoring();
            this.isInitialized = true;
            console.log('✅ AMPL Rebase Protection Monitor initialized successfully');
        } else {
    ...
    enhanceRebasePanel() {
        ...
        // Enhanced rebase protection panel with actual monitoring
        const rebaseHTML = `
            <div class="enhanced-rebase-content">
                ...
                <!-- Enhanced Progress Bar with Real Data -->
                <div class="progress-container">
                    ...
                    <div class="progress-bar-container">
                        ...
                        <div class="progress-segments">
                            <div class="segment" data-level="25"></div>
                            <div class="segment" data-level="50"></div>
                            <div class="segment" data-level="75"></div>
                        </div>
                    </div>
                    <div class="progress-percentage" id="rebase-progress-percentage">0% Safe</div>
                </div>
                
                <!-- Real Holdings Tracker -->
                ...
                <div class="rebase-info">
                    ...
                    <div class="info-row">
                        ...
                        <span class="info-value deviation" id="price-deviation-display">0.00%</span>
                    </div>
                    <div class="info-row">
    ...
    applyStyles() {
        ...
        style.id = 'ampl-rebase-protection-styles';
        style.textContent = `
            /* Enhanced Rebase Protection Styles */
            .enhanced-rebase-content {
                width: 100%;
                height: 100%;
                display: flex;
        ...
        const existingStyle = document.getElementById('ampl-rebase-protection-styles');
        if (existingStyle) {
    ...
    bindEventListeners() {
        console.log('🔗 Binding rebase protection event listeners...');
        ...
        console.log('✅ Rebase protection event listeners bound');
    }
    ...
    async fetchAMPLHoldings() {
        try {
            ...
            if (window.amplRebalancingSystemNew && window.amplRebalancingSystemNew.coins) {
                const amplCoin = window.amplRebalancingSystemNew.coins.AMPL;
                if (amplCoin) {
                ...
                if (response.ok) {
                    ...
                    if (data.positions && data.positions.AMPL) {
                        this.rebaseData.amplQuantity = data.positions.AMPL.quantity || 0;
                        this.rebaseData.originalPurchase = (data.positions.AMPL.quantity || 0) * (data.positions.AMPL.avgPrice || 0);
                    }
                }
            }
    ...
    async fetchAMPLPrice() {
        try {
            // Try multiple sources for AMPL price
            const pricePromises = [
                this.fetchSupabasePrice(),
            ...
            for (const result of results) {
                if (result.status === 'fulfilled' && result.value) {
                    this.rebaseData.amplPrice = result.value;
                    return;
                }
            }
...
// Initialize the rebase protection monitor
const amplRebaseProtectionMonitor = new AMPLRebaseProtectionMonitor();

console.log('🛡️ AMPL Rebase Protection Monitor loaded successfully');
